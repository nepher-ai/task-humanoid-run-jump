...
def randomize_hl_course(
    ...
    # Clear obstacle buffers for these envs.
    ...
    # Write poses to obstacle rigid bodies.
    ...
    for k in range(max_slots):
        ...
        if active.any():
            ...
            # Obstacle centre along +x: front face at obstacle_x, thickness along x.
            # Cuboid size is (thickness, lateral, height) → half-extents.
            # Front face at s_f, centre at s_f + t/2.
            ...
            pos[a_ids, 2] = h - cuboid_half_height  # bury so top = h

        # write_root_pose_to_sim expects [N, 7] = pos(3) + quat(4)
    ...
    # Refresh derived course features for the active obstacle.
